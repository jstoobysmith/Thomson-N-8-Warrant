...
`Tri5b/`) and against the numbers at `pivotsNum` (scripts in `plans/` are described where used).
...
## 3. Numbers everyone should know (at `pivotsNum`, double precision; `plans/survey.py` and
`plans/check_kernel.py` reproduce them — run both from the repository root)

| quantity | value |
|---|---|
| chords `A, D, N, F` | `1.17125, 1.65639, 1.28769, 1.89689` |
| `‖pivotMatrix‖∞`, `‖N‖∞`, `‖I − N·pivotMatrix‖∞` | `132`, `3.9·10³`, `1.3·10⁻¹⁰` (`threepoint/task1a_pivotmatrix.json`, `N` has 12-digit rational entries) |
...
* New Lean lives in `Thomson/Interval/` (T0), `Thomson/ThreePoint/Kernel*.lean` (1c),
  `Thomson/ThreePoint/Cert/` (per-task certificate files), `Thomson/Tri5b/` (Task 5).  Add every
  new module to `Thomson.lean`.
* Generated Lean files carry the header `-- GENERATED by threepoint/<script>.py; do not edit` and
  the generating script is committed in `threepoint/`.
...
* Do not touch `threepoint/task1_*.py` outputs or `CertData.lean` except through
