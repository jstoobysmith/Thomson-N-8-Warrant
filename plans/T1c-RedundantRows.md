...
`Thomson/ThreePoint/Cert/Redundant.lean` (the two theorems).  No numerics at all.*

## 0. What is actually true (verified symbolically, `plans/check_kernel.py`)
...
Script `threepoint/kernel_lean.py` (start from `plans/check_kernel.py`, which already parses
