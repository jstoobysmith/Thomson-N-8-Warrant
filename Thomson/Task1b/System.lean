...
/-- One column of the enclosure of `pivotMatrix`, from the tabulated tensor of `Gh j`. -/
def colOf (A : ITA) : Array Itv := Array.ofFn (n := 24) fun i : Fin 24 => entAt A.get i

/-- The enclosure of `pivotMatrix`, column by column.  A closed term: built once. -/
def MA : Array (Array Itv) := Array.ofFn (n := 24) fun j : Fin 24 => colOf (GA j)

def Mget (i j : ℕ) : Itv := (MA.getD j #[]).getD i Itv.zero

theorem mem_Mget (i j : Fin 24) : (Mget i j).Mem (pivotMatrix i j) := by
  have e : Mget i j = entAt (GA j).get i := by
    simp only [Mget, MA, colOf, Array.getD_eq_getD_getElem?, Array.getElem?_ofFn, i.isLt, j.isLt,
      dif_pos, Option.getD_some]
  rw [e]; exact mem_entAt (ITMem_GA j) i

/-- The enclosure of the residual. -/
def resOf (A : ITA) : Array Itv := Array.ofFn (n := 24) fun i : Fin 24 => resAt A.get i

def RA : Array Itv := resOf FA

def Rget (i : ℕ) : Itv := RA.getD i Itv.zero

theorem mem_Rget (i : Fin 24) : (Rget i).Mem (rowFun i pivotsNum) := by
  have e : Rget i = resAt FA.get i := by
    simp only [Rget, RA, resOf, Array.getD_eq_getD_getElem?, Array.getElem?_ofFn, i.isLt,
      dif_pos, Option.getD_some]
  rw [e]; exact mem_resAt ITMem_FA i
