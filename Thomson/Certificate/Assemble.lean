...
import Thomson.TriangleLocal.Glue
...
|---|---|---|---|
| 1a | `Task1a` (`det ≠ 0`) | **proved** (`decide +kernel`) | `Task1b` library: `Thomson.Task1b.task1a` |
| 1b | `Task1b` (pivots to `pivotEps = 10⁻¹²`) | **proved**, to `2·10⁻²¹` (`decide +kernel`) | `Task1b` library: `Thomson.Task1b.task1b` |
| 1c(i) | `row_pairVal_A` | **proved** from 1a (algebra: `ring`) | `Thomson.Certificate.Redundant`, `Slack`, `Kernel` |
| 1c(ii) | `row_triD_FDN_v` | **proved** from 1a (algebra + calculus) | `Thomson.Certificate.Redundant` |
...
| 5b | `Task5b` | **proved** from 1b, 5a (`decide +kernel`) | `Tri5b` library: `Thomson.Tri5b.task5b_of_task5a` |
| 6 | `side_conditions` | **proved** | below |

**This file, and the whole default build, is `sorry`-free.**  Task 5a is proved by the certificate
of `Thomson/TriangleLocal/` (`decide +kernel` only), imported above.

The three tasks with expensive certificates (1a, 1b, 5b) are kept out of the default build: their
libraries (`lake build Task1b`; `lake build Tri5b`, some 30 CPU-hours of `decide +kernel`) import
this file.  Nothing uses `native_decide`.  So here
...
* chord range `[9619/10000, 2]`: `√(2 − 2·0.5373) = 0.96197…` rounded down (`pair_of_poly`,
  `tri_of_poly`).  An earlier `24/25` made Task 5 false (`Thomson.TriangleGlobal.Domain`);
...
`Fh`, `Hp`, `pivots`, `certH` (`Thomson.Certificate.Linear`, data in `CertData.lean`): the
...
**Numbers** (`scripts/threepoint/task1_design2.py`; `docs/history/README.md` §3): `‖pivotMatrix‖∞ ≈ 1.3·10²`,
...
`scripts/threepoint/task1_design.json` (first column halved, see `Thomson/Pivots/README.md`),
...
enclosed with the sharp enclosures of `u*, √2, r, s₂, s₄` (`Thomson.Antiprism.Enclosure`,
`Thomson.Antiprism.SharpChords`). -/
def Task1b : Prop := ∀ j, |pivots j - pivotsNum j| ≤ pivotEps

/-! ### Consequences of Task 1b: the pivots may be replaced by `pivotsNum`

Everything built from the certificate is affine in the pivots (`Thomson.Certificate.Linear`), so
Task 1b bounds the error made by evaluating at the rational reference vector `pivotsNum` instead —
by `pivotEps` times an explicit rational ℓ¹-norm (`Thomson.Certificate.Perturb`).  Note that they
...
(`Thomson.Certificate.Redundant`).  The slack identity along the antiprism family
(`Thomson.Certificate.Slack`, the algebra of `three_point_bound` as an equality): for every `p` and
`u ∈ [0, 1)`,
`2·E(u) − (64a₀ − 8(a₀+a₁) − 8F(1,1,1)) = pairSum p u + triSum p u + bvTerm p u`,
with `bvTerm p u = Σ_k ⟨Hp p k, B_kᵀ Acomb_k(u) B_k⟩`.  **Kernel lemma**
(`Thomson.Certificate.Kernel`, generated by `scripts/threepoint/kernel_lean.py`):
...
**Proved** (`Thomson.Certificate.Redundant`): differentiate the slack identity along the family at
...
(`scripts/threepoint/psd_gen.py`): `D ≥ 0`, and `N ≈ μ_k·P` is diagonally dominant for every `p` within
...
*Plan*: `docs/history/T5a-TriLocal.md`.
...
are affine in `pivots`; the Taylor-shift machinery of `Thomson/TriangleGlobal/` computes them as interval
...
`decide +kernel`.  See `Thomson/TriangleGlobal/README.md`. -/
def Task5b : Prop :=
...
proved outside the default build by `decide +kernel` (`Complete.lean` discharges them). -/
