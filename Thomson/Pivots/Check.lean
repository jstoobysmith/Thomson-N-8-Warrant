...
`η ≤ 10⁻¹¹` (it is `5.25·10⁻¹²`) and `ρ ≤ 10⁻²¹` (it is `4.0·10⁻²²`).  Everything expensive has
already been done: `Mtab` and `Rtab` are literal tables (`KSystem`), so this last step is `≈1.4·10⁴`
interval products and the kernel does it in seconds. -/
...
def NMI (i j : Fin 24) : Itv :=
  Itv.sumL ((List.finRange 24).map fun k : Fin 24 => Itv.mul (Itv.cst (Nq i k)) (Mtab k j))
...
def NRI (i : Fin 24) : Itv :=
  Itv.sumL ((List.finRange 24).map fun k : Fin 24 => Itv.mul (Itv.cst (Nq i k)) (Rtab k))
...
theorem check_ok : checkOK = true := by decide +kernel
...
theorem mem_NMI (i j : Fin 24) : (NMI i j).Mem ((Nmat * pivotMatrix) i j) := by
  ...
  exact Itv.mem_mul (Itv.mem_cst _) (mem_Mtab k j)
...
theorem mem_NRI (i : Fin 24) :
  ...
  unfold NRI
  refine Itv.mem_sum_fin _ _ fun k => ?_
  exact Itv.mem_mul (Itv.mem_cst _) (mem_Rtab k)
