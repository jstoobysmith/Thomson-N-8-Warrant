import Thomson.Unique.PairStrictCerts
...
`UniquenessPlan.md`, U3.  On `[24/25, 2]`, `pairP pivots s > 0` unless `s` is one of the four
chords `A, D, N, F`.  The proof is that of `Thomson.Pair.pairP_nonneg_of` with the strict sweeps
(`Thomson.Unique.PairStrict`, `Thomson.Unique.PairStrictCerts`): `Φ > 0` on each side of the chord
...
(from Tasks 1a, 1b; `UniquenessPlan.md`, U3). -/
