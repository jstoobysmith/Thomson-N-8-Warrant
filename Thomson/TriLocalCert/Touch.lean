...
Rational (`10⁻⁴⁰`-grid) enclosures of the four chord lengths, from `Thomson.Tri5b.CubeData`'s
`chordA_bounds`/`chordD_bounds`/`chordN_bounds`/`chordF_bounds` (25–34 digit rational bounds), and
of the five touching types, from `Thomson.ThreePoint.Sharp`'s `touchType_zero`..`touchType_four`. -/
