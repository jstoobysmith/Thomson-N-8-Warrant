...
argument of `Thomson.Antiprism.Enclosure` (the sign of `antiprismEnergy'` at two rationals straddling `u*`,
plus the strict monotonicity of `antiprismEnergy'`), with `√2` to `45` digits.

The same digits are what a sharpened Task 1b needs (see `Thomson.TriangleGlobal.Reduce`). -/
