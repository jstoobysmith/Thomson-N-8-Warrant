...
`Thomson.ThreePoint.Tasks`, Task 1b: the residual bound for the pivot enclosure is limited only by
...
`Thomson.Derivative` (the sign of `antiprismEnergy'` at two rationals straddling `u*`). -/
