...
(Antiprism/UStarSharp) u* to 32 digits
(Numerics/Interval)  fixed-point intervals         scale 10⁻⁴⁰, exact for all the certificate's data
(Numerics/TaylorModel) quadratic Taylor models       value + gradient + Hessian + radius, and `loBound`
(Numerics/Tensor) coefficient tensors           the Taylor shift `ev (shift c a b d) = ev c (a+·) …`
(Numerics/TensorEngine) interval tensors              shift, `toTM`, `box_sound`, `inv_sqrt_tangent`
...
(Numerics/PolyList) nested-list polynomials       Horner's Taylor shift, box normalisation, `Mem3`
(Numerics/ListTaylorModel) Taylor model of a nested list `toTML`, `toTML_mem`
KernelLeaf.lean   the kernel leaf and checker   `kleafCheck`, `kskipOK`, `kbcheck_sound`
KernelTable.lean  the tensor of `F` as a list   from `TriLocalCert.CFData` (`decide +kernel`)
Covering/Part000–636  the covering, in 637 pieces   4 leaves per `decide +kernel`, ≤ 128 per file
Covering/Certificate  the pieces, recombined        `kcert_cover`, by `covers_split`
Final.lean        the theorem                   `tri_nonneg`, `triP_global`
```

### The kernel layer (`K*.lean`)

The interpreter-oriented files (`Engine`, `Leaf`, `Skip`) still hold the mathematics — the
Taylor-model bound, the tangent minorant, the tree soundness — but the covering is *checked* by
`decide +kernel`, not `native_decide`, so nothing depends on `Lean.ofReduceBool`.  The kernel
cannot run `Array.ofFn` or `Int.sqrt` (well-founded recursion), pays for every array read
(arrays are lists to it) and does about 1,600 `Itv.mul` per second.  So the kernel layer stores the
tensor as `List (List (List Itv))`, shifts it by Horner's form of the Taylor shift
(`p(x+a) = c₀ + (x+a)·q(x+a)`, multiplications only by the exact point `a/SCALE`, `Itv.mulC`),
normalises to the box with exact rational scalings (`Itv.mulQ`), reads the Taylor model off the
result (`toTML`), shifts `gram` separately so that the fourteen S-procedure multipliers cost one
shift of `F`, and takes the tangent chord from a fuelled Newton iteration (`nsqrt`).  One leaf is
≈ 11,000 point multiplications, about 4 s in the kernel; the tree (29,674 leaves, the same bit
stream as before, `scripts/threepoint/tri5b_tree2.pkl`) is some 30 CPU-hours.  Memory and time of one
`decide +kernel` grow faster than linearly with the work in it (4 leaves ≈ 17 s, 2 GB; 32 leaves
≈ 10 min, 8 GB), hence four leaves per theorem and 128 per file.  `scripts/threepoint/tri5b_kmirror.py`
mirrors the kernel arithmetic bit for bit and `scripts/threepoint/tri5b_kparts.py` emits the pieces.
...
`decide +kernel` (the kernel layer above), so the axioms are `propext, Classical.choice, Quot.sound`
only.

## What remains

**Only Task 5a** (`TriLocal`, the local cubes of radius `1/500`).  Task 1b at `10⁻¹²` is proved in
the `Task1b` library (`Thomson/Pivots/`, to `2·10⁻²¹`), and `Discharge.lean` combines the two:
...
* **Cut the covering into pieces.**  A covering is a union, so a box may be covered piece by piece
  (`covers_split`).  The tree is cut greedily into subtrees of `≤ 128` leaves, one module each,
  and within a module into groups of `≤ 4` leaves, one `decide +kernel` each (originally 107
  modules with one `native_decide` each, about an hour of CPU).

## Reproducing and re-checking

`scripts/threepoint/tri5b_*.py` regenerate everything: `tri5b_mirror.py` is a bit-for-bit Python copy of
...
interval multiplications per second measured for `decide +kernel` in `docs/history/README.md` §1.1 —
...
the decision table of `docs/history/T5b-TriGlobal.md` §4 (which assumed `10⁵`–`10⁶` boxes from the
centred form).
