...
`docs/history/T5a-TriLocal.md` §6 ("Step 4 — assembly") isolates a purely calculus statement, independent
...
`docs/history/T5a-TriLocal.md`): prove it once, in general, then instantiate it five times (once per
...
`scripts/threepoint/tri_local.py` in the task plan) and *checked* (a `Thomson.Tri5b.Itv`-based interval
...
`docs/history/README.md` notes does not exist yet, or an equivalent bespoke development for `triP`
...
This packages `taylor3_right`'s cubic bound exactly as `docs/history/T5a-TriLocal.md` §6 describes:
