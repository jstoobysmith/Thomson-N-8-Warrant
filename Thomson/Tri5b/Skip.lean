...
`native_decide` runs in the interpreter, at `≈10⁻¹` s per box; the whole tree is an hour on one
core.  A box may be covered piece by piece instead, each piece checked by its own
`native_decide` in its own module — which `lake` then builds in parallel. -/
