import Thomson.Uniqueness.Defs

/-! # Uniqueness, step U5: the combinatorial classification

`docs/uniqueness.md`, U5.  An edge colouring `c` of `K₈` by the four chords whose every triangle is
...
quadruples `alWits` (`python3 docs/history/unique_enum.py sorted lean` reproduces both lists). -/
