...
import Thomson.Uniqueness.Congruent

/-! # U6: geometry — the chord inner products, no aligned quadruple, the antiprism Gram table

`docs/uniqueness.md`, step U6.  Colours `A = 0, D = 1, N = 2, F = 3` as in `Thomson.chord`. -/
...
This is **not** the labelling of `docs/history/unique_enum.py` (there `vᵢ` is near `wᵢ, w_{i+1}`); see
...
/-- The antiprism pattern in the labelling of `docs/uniqueness.md` / `docs/history/unique_enum.py`
