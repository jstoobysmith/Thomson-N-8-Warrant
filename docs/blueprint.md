...
| `Thomson/ThreePoint/CertData.lean` | **generated data tables** (`scripts/threepoint/task1_emit_lean.py`): scaled exact kernel bases `B k`, fixed rationals `a0Fix, a1Fix, lamFix, Hfix`, the 24 pivot `slot`s, `pivotsNum` |
...
finish it.  Scripts and data: [`scripts/threepoint/`](scripts/threepoint/), see its README.)*
...
### 12.3 The structure of the sharp certificate (`scripts/threepoint/certificate_numerical.json`)
...
**Step 1 of §12.5 is done** (`scripts/threepoint/exact_kernels.py`, output `kernel_vectors_exact.json`):
...
**Step 2, first pass** (`scripts/threepoint/project_certificate.py`): projecting the numerical `F_k`
...
**GO/NO-GO test — passed** (`scripts/threepoint/gonogo.py`, output `certificate_tight.json`).  Instead of
...
**Update 2026-09-10 (evening): per-task proof plans are in [`docs/history/`](docs/history/).**  Start with
[`docs/history/README.md`](docs/history/README.md): it records two facts that change the sketches below —
...
interval kernel (`docs/history/T0-Infrastructure.md`), in the Lean kernel for Tasks 1a, 1b, 2, 4, 5a and
...
`(u − u*)²` (`docs/history/check_kernel.py`).
...
`CertData.lean` generated from `scripts/threepoint/task1_design.json` by `task1_emit_lean.py`):
