# Uniqueness (docs/uniqueness.md, step U5): enumerate the edge colourings of K8 by the four
...
#
#   python3 docs/history/unique_enum.py          full search:      224 922 nodes, 5040 leaves
#   python3 docs/history/unique_enum.py sorted   vertex-0 sorted:    1 878 nodes,    8 leaves
#   python3 docs/history/unique_enum.py sorted lean   also print the leaf witnesses stored in
#                                         `Thomson/Uniqueness/Graph.lean` (`apWits`, `alWits`)
#
...
if len(sys.argv) > 2 and sys.argv[2] == 'lean':
    # Witnesses for `leafL` in `Thomson/Uniqueness/Graph.lean`: a relabelling sigma (as the list
