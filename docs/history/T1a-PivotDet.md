...
*Needs: T0 (§1–2, §4 without pivots, §5 `GhE`).  Output: `Thomson/TriangleLocal/Calculus/PivotDet.lean`
...
Let `N` be the rational 24×24 matrix in `scripts/threepoint/task1a_pivotmatrix.json` (`N_num / den`,
...
such.  Define the mirrored row entries (`Thomson/TriangleLocal/Calculus/PivotData.lean`, hand-written, 24
...
Run `scripts/threepoint/pivotmatrix_lean.py`'s arithmetic in *interval* form (Python `fractions` with the
