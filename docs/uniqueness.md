...
`Thomson/Pair/`, `Thomson/TriangleGlobal/`, `Thomson/Main.lean`, `Thomson/Complete.lean`) and blueprint §12.
Numbers were checked with `docs/history/survey.py`'s evaluator and `docs/history/unique_enum.py`.*
...
exactly two solutions up to relabelling (`docs/history/unique_enum.py`): the antiprism, and an "aligned"
...
## 2. Numbers everyone should know (at `pivotsNum`; `docs/history/survey.py` reproduces them)
...
re-checks), then U1–U2, U5, U6 in parallel; U7 assembles.  All new code goes in `Thomson/Uniqueness/`
...
### U1 — the three-point bound as an identity (`Thomson/Uniqueness/Identity.lean`)
...
### U2 — the concrete certificate and the chord/triangle conclusions (`Thomson/Uniqueness/Tight.lean`)
...
(it will not: the sweeps meet where `Φ ≈ 4·10⁻⁶ … 10⁻⁴`), regenerate with `scripts/threepoint/pair_gen.py`.
...
```lean
-- Thomson/TriangleGlobal/Final.lean
theorem numCertU_final (hclose : ∀ j, |pivots j - pivotsNum j| ≤ 1 / 10 ^ 12) :
    NumCertU (fun _ => 1 / 500) (1 / 10 ^ 10)
-- Thomson/ThreePoint/Tasks.lean (default build; `Thomson.TriangleGlobal.Main` is a light module)
def Task5bStrict : Prop := Tri5b.NumCertU (fun _ => 1 / 500) (1 / 10 ^ 10)
-- Thomson/TriangleGlobal/Task5b.lean
...
`Complete.lean`, like `Task5b`.  Then (main library, `Thomson/Uniqueness/Chords.lean`):
...
### U5 — the combinatorial classification (`Thomson/Uniqueness/Graph.lean`, kernel `decide`)
...
2. **A verified search (80 lines).**  Edges `e₀, …, e₂₇` are the pairs `(i, j)`, `i < j`, ordered by
   ...
   `python3 docs/history/unique_enum.py sorted`), each stored with either an explicit permutation
...
### U6 — geometry: no aligned quadruple, and Gram matrix ⟹ congruence (`Thomson/Uniqueness/Gram.lean`)
...
### U7 — assembly (`Thomson/Uniqueness/Main.lean`, `Thomson/Main.lean`, `Thomson/Complete.lean`)
...
* `python3 docs/history/unique_enum.py sorted` prints `nodes 1878, leaves 8` and the eight leaves stored in
...
* **Strict sweep fails a step.**  Only possible by integer rounding of a `p` that is genuinely tiny;
  ...
  the cells with `scripts/threepoint/pair_gen.py` and a requested margin.
