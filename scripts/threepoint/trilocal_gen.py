"""[Task 5a] Exact Python mirror of `Thomson/TriangleLocal/*.lean`: the per-type data (enclosures of the
...
Usage (from the repository root):  python3 scripts/threepoint/trilocal_gen.py
"""
...
# ---------- the tensor of F (Thomson.TriangleLocal.CFData: CFt) ----------
_cf = pickle.load(open(os.path.join(ROOT, "scripts", "threepoint", "cfarr.pkl"), "rb"))
CF = [[[tuple(_cf[81 * i + 9 * j + k]) for k in range(9)] for j in range(9)] for i in range(9)]

# ---------- the touching types (Thomson.TriangleLocal.Touch: Tt) ----------
...
# ---------- emitting Lean ----------
HDR = "-- GENERATED by scripts/threepoint/trilocal_gen.py; do not edit\n"
def itv(I): return "⟨%d, %d⟩" % I
def l1(L): return "[" + ", ".join(itv(I) for I in L) + "]"
def l2(L): return "[" + ",\n    ".join(l1(r) for r in L) + "]"
def l3(L): return "[" + ",\n   ".join(l2(r) for r in L) + "]"

TYPE = HDR + """import Thomson.TriangleLocal.Compute
import Thomson.TriangleLocal.Touch
...
FACE = HDR + """import Thomson.TriangleLocal.Types.Type{m}
...
GLUE = HDR + """{imports}
import Thomson.TriangleLocal.Final
import Thomson.TriangleLocal.Calculus.TriLocalFinal
import Thomson.TriangleLocal.Calculus.TriLocalPolarBridge
...
CFDATA = HDR + """import Thomson.TriangleGlobal.CertIntervals
...
def emit(outdir):
    ...
    for m, (T0, T1, T2) in enumerate(TT):
        ...
        open(os.path.join(outdir, "Types", "Type%d.lean" % m), "w").write(TYPE.format(
        ...
        for f in range(3):
            for sg in (True, False):
                ...
                open(os.path.join(outdir, "Types", "Face%d_%d%s.lean" % (m, f, "p" if sg else "n")), "w").write(
