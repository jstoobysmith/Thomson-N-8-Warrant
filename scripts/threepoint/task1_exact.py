...
def ratfun_to_K(expr_str):
    ...
    Pn=sp.Poly(sp.expand(n_),_u); Pd=sp.Poly(sp.expand(d_),_u)
    def toK(P):
        tot=K()
        for (k,),c in P.terms():
            c=sp.Rational(c); tot=tot+U_**k*K.const(Fr(int(c.p),int(c.q)))
        return tot
    return toK(Pn)*inv(toK(Pd))
V={}
for k in range(KMAX+1):
    V[k]=[ratfun_to_K(s) for s in kv[str(k)]]
log("exact kernel vectors in K")
# basis B_k of v_k^perp over K: v normalised with v[0]=1 -> columns e_i - v_i e_0 (i>=1)
def basis_perp(v):
    n=len(v)
    p0=next(i for i in range(n) if v[i].d)          # first nonzero coordinate (v normalised there = 1)
    assert v[p0].d=={(0,0,0,0,0):Fr(1)}, "kernel vector not normalised at its pivot"
    cols=[]
    for i in range(n):
        if i==p0: continue
        col=[K() for _ in range(n)]; col[i]=K.const(1); col[p0]=K.const(0)-v[i]
        cols.append(col)
    return cols       # list of n-vectors over K spanning v^perp; F = sum_{a,b} H[a,b] col_a col_b^T
...
import pickle; pickle.dump(dict(rows=[({p:v.d for p,v in row.items()},rhs.d) for row,rhs in rows],x0=[str(v) for v in x0],names=names,idx=idx),open(os.path.join(here,'task1_rows.pkl'),'wb'))
