...
EPSMIN=float(sys.argv[6]) if len(sys.argv)>6 else 5e-4
eta=cp.Variable(); cons.append(eta>=0)
...
print(f"Hessian margin eta is a VARIABLE (maximised); inequality margin eps >= {EPSMIN}")
...
t0=time.time(); cons.append(eps>=EPSMIN); p=cp.Problem(cp.Maximize(eta),cons); p.solve(solver=cp.CLARABEL,tol_gap_abs=1e-10,tol_gap_rel=1e-10,tol_feas=1e-10,max_iter=800)
print(f"status={p.status}  eta*={(eta.value if eta.value is not None else float('nan')):.3e}  eps={(eps.value if eps.value is not None else float('nan')):.3e}  (PSD margin fixed {MU}, {len(tri)} triangle pts incl. boundary, {time.time()-t0:.0f}s)")
if eta.value is None: sys.exit(2)
Fv=[W[k]@H[k].value@W[k].T for k in range(K+1)]
json.dump(dict(a0=float(a0.value),a1=float(a1.value),lam=float(lam.value),mu=MU,eps=float(eps.value),eta=float(eta.value),
