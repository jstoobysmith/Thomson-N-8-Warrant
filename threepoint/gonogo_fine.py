...
for t in np.linspace(tlo,thi,1500): cons.append(Pexpr(np.sqrt(2-2*t))>=0)
g=np.linspace(tlo,thi,44); tri=[(u,v,t) for u in g for v in g if v>=u for t in g if t>=v and 1+2*u*v*t-u*u-v*v-t*t>=-1e-12]
...
if mu.value is not None:
    ...
    json.dump(dict(a0=float(a0.value),a1=float(a1.value),lam=float(lam.value),mu=float(mu.value),
              ...
              open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'certificate_tight_fine.json'),'w'),indent=1)
